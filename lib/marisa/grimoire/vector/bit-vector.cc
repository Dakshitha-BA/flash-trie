...
inline __host__ __device__ uint64_t ith_set_pos(uint32_t i, uint64_t val) {
...
__host__ __device__ std::size_t select_bit(std::size_t i, std::size_t bit_id, UInt64 unit) {
 #ifdef _MSC_VER
  unsigned long pos;
 ...
 #else  // _MSC_VER
 #ifdef __CUDA_ARCH__
  return bit_id + ith_set_pos(i, unit);
 #else
