...
void Trie::init_tbs_gpu(size_t max_batch_size, size_t max_num_topk,
  ...
  assert(num_tries() == 1);
  assert(node_order() == MARISA_LABEL_ORDER);

  trie_->init_tbs_gpu(max_batch_size, max_num_topk, max_num_outpos,
      max_num_out_beams, max_num_out_tokens, max_num_inter_beams,
      radix_topk_threshold);
  trie_->prefetch();  // Prefetch should be after init
}

size_t Trie::get_max_batch_size() const {
  MARISA_THROW_IF(trie_.get() == NULL, MARISA_STATE_ERROR);
  return trie_->max_batch_size();
}

void Trie::tbs(TbsInput &input) const {
  MARISA_THROW_IF(trie_.get() == NULL, MARISA_STATE_ERROR);
  trie_->tbs(input);
}

void Trie::tbs_gpu(TbsInput &input, size_t batch_id) const {
  MARISA_THROW_IF(trie_.get() == NULL, MARISA_STATE_ERROR);
  trie_->tbs_gpu(input, batch_id);
}

void Trie::tbs_gpu_batched(std::vector<TbsInput> &inputs) const {
  MARISA_THROW_IF(trie_.get() == NULL, MARISA_STATE_ERROR);
  trie_->tbs_gpu_batched(inputs);
}
