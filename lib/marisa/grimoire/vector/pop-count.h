...
class PopCount {
 public:
  __host__ __device__ explicit PopCount(UInt64 x) : value_() {
  ...
  __host__ __device__ std::size_t lo8() const {
    return (std::size_t)(value_ & 0xFFU);
  }
  __host__ __device__ std::size_t lo16() const {
    return (std::size_t)((value_ >> 8) & 0xFFU);
  }
  __host__ __device__ std::size_t lo24() const {
    return (std::size_t)((value_ >> 16) & 0xFFU);
  }
  __host__ __device__ std::size_t lo32() const {
    return (std::size_t)((value_ >> 24) & 0xFFU);
  }
  __host__ __device__ std::size_t lo40() const {
    return (std::size_t)((value_ >> 32) & 0xFFU);
  }
  __host__ __device__ std::size_t lo48() const {
    return (std::size_t)((value_ >> 40) & 0xFFU);
  }
  __host__ __device__ std::size_t lo56() const {
    return (std::size_t)((value_ >> 48) & 0xFFU);
  }
  __host__ __device__ std::size_t lo64() const {
    return (std::size_t)((value_ >> 56) & 0xFFU);
  }

  __host__ __device__ static std::size_t count(UInt64 x) {
#ifdef __CUDA_ARCH__
