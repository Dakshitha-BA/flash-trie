...
static constexpr Label TERMINAL = -1u;
  ...
  switch (mode) {
    case MARISA_TEXT_TAIL: {
      for (std::size_t i = 0; i < entries.size(); ++i) {
        const Label * const ptr = entries[i].ptr();
        const std::size_t length = entries[i].length();
        for (std::size_t j = 0; j < length; ++j) {
          if (ptr[j] == TERMINAL) {
            MARISA_DEBUG_IF(true, MARISA_BOUND_ERROR);
...
void Tail::restore(Agent &agent, std::size_t offset) const {
  ...
  if (end_flags_.empty()) {
    for (const Label *ptr = &buf_[offset]; *ptr != TERMINAL; ++ptr) {
...
__device__ bool Tail::match(Label token, std::size_t &offset, bool &end) const {
  bool match = buf_[offset] == token;
  if (match) {
    offset++;
    end =
        end_flags_.empty() ? buf_[offset] == TERMINAL : end_flags_[offset - 1];
  }
  return match;
}

__device__ Label Tail::next_token(std::size_t &offset, bool &end) const {
  if (end_flags_.empty()) {
    assert(offset + 1 < buf_.size());
  } else {
    assert(offset < buf_.size());
  }
  auto token = buf_[offset++];
  end = end_flags_.empty() ? buf_[offset] == TERMINAL : end_flags_[offset - 1];
  return token;
}
...
bool Tail::prefix_match(Agent &agent, std::size_t offset) const {
  ...
  State &state = agent.state();
  if (end_flags_.empty()) {
    const Label *ptr = &buf_[offset] - state.query_pos();
    do {
      if (ptr[state.query_pos()] != agent.query()[state.query_pos()]) {
        return false;
      }
