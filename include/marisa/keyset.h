...
class Keyset {
 public:
  ...
  void push_back(const Key &key, Label end_marker);

  void push_back(const std::vector<Label>& str, float weight = 1.0) {
    push_back(str.data(), str.size(), weight);
  }
  void push_back(const Label *ptr, std::size_t length, float weight = 1.0);
 ...
 private:
  scoped_array<scoped_array<Label> > base_blocks_;
  std::size_t base_blocks_size_;
  std::size_t base_blocks_capacity_;
  scoped_array<scoped_array<Label> > extra_blocks_;
