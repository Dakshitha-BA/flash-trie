...
class RankIndex {
 public:
  ...
  void set_rel7(std::size_t value) {
   ...
   __host__ __device__ std::size_t abs() const {
    return abs_;
  }
  __host__ __device__ std::size_t rel1() const {
    return rel_lo_ & 0x7FU;
  }
  __host__ __device__ std::size_t rel2() const {
    return (rel_lo_ >> 7) & 0xFFU;
  }
  __host__ __device__ std::size_t rel3() const {
    return (rel_lo_ >> 15) & 0xFFU;
  }
  __host__ __device__ std::size_t rel4() const {
    return (rel_lo_ >> 23) & 0x1FFU;
  }
  __host__ __device__ std::size_t rel5() const {
    return rel_hi_ & 0x1FFU;
  }
  __host__ __device__ std::size_t rel6() const {
    return (rel_hi_ >> 9) & 0x1FFU;
  }
  __host__ __device__ std::size_t rel7() const {
